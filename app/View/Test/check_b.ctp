...
<?php
	echo $this->Form->create('TestLog',array(
			'type' => 'post','url' => 'result_b'));

	foreach($test_b as $key => $value){
		echo "<div class='hero-unit'>";
		$quiz = "";
		$quiz .= "[".($key+1)." / 50問]";
		$quiz .= trim($value['AutoQuiz']['sentence']); //問題文
		$quiz .= '<br />';


		//チェックボックスが未選択のものの値は0とする
		if($check['TestLog']['answer'][$key] == null){
			$check['TestLog']['answer'][$key] = "";
		}

		$quiz .= $check['TestLog']['answer'][$key];

		$quiz .= $this->Form->input('TestLog.answer.'.$key, array(
		 			'type' => 'hidden',
		 			'value' => $check['TestLog']['answer'][$key]));
		// for ($i=1; $i <= 4; $i++) { 
